...
import {getAllPosts} from '../app/posts/duck/operations'
import actions from '../app/posts/duck/actions'
...
class UserDetails extends React.Component {
  ...
  handleSaveClick = (e) => {
    ...
    this.userPosts = this.props.posts.list.filter(post => post.userId == this.user.id);
  ...
  deletePost = (id) => {
    console.log(id);
    const idx = this.userPosts.findIndex(post => post.id == id)
    this.userPosts.slice(idx, 1)
    this.postsList = this.userPosts.map(post => (
      <Post key={post.id} post={post} user={this.user} />))
    this.setState(
      {title: ''}
  ...
  postsList = this.userPosts.map(post => (
    <Post key={post.id} post={post} user={this.user} deletePost={this.deletePost}/>
  ));
  ...
  return (
    <>
      <header>
        ...
        <div className="icon">
          <i onClick= {() => this.openModal()}className="fa fa-plus-circle fa-3x" />
        </div>
      </header>
      <main>
